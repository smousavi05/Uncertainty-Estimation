...
textread('../../../data/beroza/mmousavi/branch-adp/Llenos_max_NSH_CEUS.5Hz.txt','%f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f');
...
all = dir('../../../data/beroza/mmousavi/branch-infv5Hz');
...
for bnum = 1:numel(branches);
    b = bw(bnum);
       cc = cc+1
    [w1,w2,w3,w4,w5,w6,w7,w8,w9,w10,w11,w12,w13,w14,w15,w16,w17,w18,w19,w20,w21,w22] = ...
    textread(sprintf('../../../data/beroza/mmousavi/branch-infv5Hz/%s',branches(bnum).name),'%f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f');
...
[w1,w2,w3,w4,w5,w6,w7,w8,w9,w10,w11,w12,w13,w14,w15,w16,w17,w18,w19,w20,w21,w22] = ...
    textread('hazardCurve16_5Hz.txt','%f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f');
...
delete('one16Percent_1YrModel.5Hz.1pc1.txt')
fileID = fopen('one16Percent_1YrModel.5Hz.1pc1.txt','w');
for k = 1 : length(haz)
fprintf(fileID,'%3.3f %2.3f %f\n',haz(k,:));
end
fclose(fileID);

delete('one16Percent_1YrModel.5Hz.1pc2.txt')
fileID = fopen('one16Percent_1YrModel.5Hz.1pc2.txt','w');
for k = 1 : length(ucr)
fprintf(fileID,'%3.3f %2.3f %f\n',ucr(k,:));
end
fclose(fileID);

delete('one16Percent_1YrModel.5Hz.1pc3.txt')
fileID = fopen('one16Percent_1YrModel.5Hz.1pc3.txt','w');
for k = 1 : length(haz)
fprintf(fileID,'%3.3f %2.3f %f\n',ucrP(k,:));
end
fclose(fileID);
...
delete('2016_5Hz_Boot_StD.txt')
fileID = fopen('2016_5Hz_Boot_StD.txt','w');
for k = 1 : length(haz)
