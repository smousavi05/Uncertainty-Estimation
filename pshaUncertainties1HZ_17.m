...
textread('../../../data/beroza/mmousavi/17branch-adp/Llenos_max_NSH_CEUS_RS.1hz.txt','%f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f');
...
all = dir('../../../data/beroza/mmousavi/17branch-infv1Hz');
...
for bnum = 1:numel(branches);
    b = bw(bnum);
       cc = cc+1
    [w1,w2,w3,w4,w5,w6,w7,w8,w9,w10,w11,w12,w13,w14,w15,w16,w17,w18,w19,w20,w21,w22] = ...
    textread(sprintf('../../../data/beroza/mmousavi/17branch-infv1Hz/%s',branches(bnum).name),'%f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f');
...
[w1,w2,w3,w4,w5,w6,w7,w8,w9,w10,w11,w12,w13,w14,w15,w16,w17,w18,w19,w20,w21,w22] = ...
    textread('hazardCurve17_1Hz.txt','%f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f');
...
delete('one17Percent_1YrModel.1Hz.1pc1.txt')
fileID = fopen('one17Percent_1YrModel.1Hz.1pc1.txt','w');
for k = 1 : length(haz)
fprintf(fileID,'%3.3f %2.3f %f\n',haz(k,:));
end
fclose(fileID);

delete('one17Percent_1YrModel.1Hz.1pc2.txt')
fileID = fopen('one17Percent_1YrModel.1Hz.1pc2.txt','w');
for k = 1 : length(ucr)
fprintf(fileID,'%3.3f %2.3f %f\n',ucr(k,:));
end
fclose(fileID);

delete('one17Percent_1YrModel.1Hz.1pc3.txt')
fileID = fopen('one17Percent_1YrModel.1Hz.1pc3.txt','w');
for k = 1 : length(haz)
fprintf(fileID,'%3.3f %2.3f %f\n',ucrP(k,:));
end
fclose(fileID);

delete('2017_1Hz_Boot_StD.txt')
fileID = fopen('2017_1Hz_Boot_StD.txt','w');
for k = 1 : length(haz)
